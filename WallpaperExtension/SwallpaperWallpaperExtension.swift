//  Swallpaper Wallpaper Extension
...
@main
final class SwallpaperWallpaperExtension: NSObject, AppExtension {
    ...
    private func observeAppTermination() {
        ...
        CFNotificationCenterAddObserver(
            ...
            "com.swallpaper.app.wallpaper.appWillTerminate" as CFString,
            nil,
            .deliverImmediately
        )
    }
    ...
    private func observeExtensionReload() {
        ...
        CFNotificationCenterAddObserver(
            ...
            "com.swallpaper.app.wallpaper.extensionReload" as CFString,
            nil,
            .deliverImmediately
        )
        ...
        CFNotificationCenterAddObserver(
            ...
            { _, _, _, _, _ in
                DispatchQueue.main.async {
                    SwallpaperWallpaperExtension.drainPendingSocketCommands(reason: "commandsChanged")
                }
            },
            "com.swallpaper.app.wallpaper.commandsChanged" as CFString,
            nil,
            .deliverImmediately
        )
    ...
    private static func pollAndHandleSocketCommand() -> Bool {
        guard let socketPath = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.com.swallpaper.app")?
    ...
    private func observeLibraryChanges() {
        ...
        CFNotificationCenterAddObserver(
            ...
            { _, _, _, _, _ in
                ...
                DispatchQueue.main.async {
                    SwallpaperWallpaperExtension.drainPendingSocketCommands(reason: "prefsChanged")
                }
            },
...
private var extLogFileURL: URL {
    if let container = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.com.swallpaper.app") {
...
func extLog(_ message: String) {
    if #available(macOS 11.0, *) {
        os_log("[SwallpaperExt] %{public}@", log: .default, type: .info, message)
    } else {
        print("[SwallpaperExt] \(message)")
    }
    // 也写入文件便于调试
